...
class Facility(Base):
    ...
    def __repr__(self):
        return "Facility name {})".format(self.abbreviation)
...
class Secteur(Base):
    ...
    def __repr__(self):
        return "Secteur name: {}".format(self.name)
...
class Act(Base):
    ...
    billingcode = Column(String) # a str of the form: ‘code,abbrv,loc,cat’
    #BOOLEANS
    ...
    def __str__(self):
    	return "Last:{}".format(self.timeStart.strftime('%d-%b-%Y'))
...
class Episode_Work(Base):
    ...
    def add_sentinel_dx(self):
        set_sentinel_dx(self)

    def __repr__(self):
        #patient = self.patient.get_decrypted_dic()
        #fname, lname, MRN = patient['fname'], patient['lname'], patient['mrn']
        #fname = self.patient.fname
        #lname = self.patient.lname
        #MRN = self.patient.mrn
        #last_act = self.act[-1].timeStart.strftime('%d-%b-%Y')
        #return "{0} {1}, MRN {2} \nlast visit: {3}".format(fname, lname, MRN, last_act)
        #return "test"
        #patient = session.query(Patient).filter_by(id = self.patient_id).first()
        #patient = patient.lname
        return "First:{}".format(self.timeStart.strftime('%d-%b-%Y'))
...
class Patient(Base):
    ...
    def add_EOW(self):
        return Episode_Work(self)

    def __str__(self):
        return "{} {} \nMRN: {}".format(self.fname, self.lname, self.mrn, self.ramq)
